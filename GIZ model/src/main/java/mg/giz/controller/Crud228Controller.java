...
@Controller
public class Crud228Controller {
	...
	@RequestMapping("/list_228")
	public String liste(Model model) {
		Long id = themeService.selectThm("2.2.8");
		...
		return "crud-form/Form_list_228";
	}
	
	@RequestMapping("/add228")
	public String addFormesForm(Model model) {
		List<Village> village = villageService.getAllVillage();
		model.addAttribute("village", village);
		return "crud-form/Form_add_228";
	}
	
	@RequestMapping("/save228")
	public String saveSensibilise(@RequestParam("code_fkt") String code_fkt,
			...
			@RequestParam("beneficierpgm_plandeviedeux") String beneficierpgm_plandeviedeux,
			RedirectAttributes redirectAttributes)
			throws ParseException {
		
		Long theme_id = themeService.selectThm("2.2.8");
		...
		Beneficierpgm beneficierpgmCreated = beneficierPgmService.create228Crud(beneficierpgm, beneficierpgm_date, beneficierpgm_lieu, beneficierpgm_plandeviedeux, selectpersonne, theme_id);
		...
		if (duplicatedCreated == true) {
			...
			return "redirect:/add228";
		}
		return "redirect:/list_228";	
	}
	@RequestMapping("/delete228/{id}")
	public String delete228(@PathVariable(name = "id") Long id) {
		beneficierPgmService.deleteBeneficier(id);
		return "redirect:/list_228";
	}
	
	@RequestMapping("/edit228/{id}/{personne_id}")
	public ModelAndView editFormateurFbs(@PathVariable(name = "id") Long id,
			@PathVariable(name = "personne_id") Long personne_id,
			Model model) throws ParseException {
		ModelAndView mav = new ModelAndView("crud-form/Form_modif_228");
		List<Village> village = villageService.getAllVillage();
		model.addAttribute("village", village);
	...
	@RequestMapping(value = "/saveModification228", method = RequestMethod.POST)
	public String saveModification228(@RequestParam("code_fkt") String code_fkt,
		...
		Beneficierpgm beneficierpgmCreated = beneficierPgmService.create228Crud(beneficierpgm, beneficierpgm_date, beneficierpgm_lieu, beneficierpgm_plandeviedeux, personne_id, theme_id);
